...
#if defined(__CUDACC__) && (CUSPARSE_VERSION >= 12000)
#define IS_CUSPARSE12_AVAILABLE() 1
#else
#define IS_CUSPARSE12_AVAILABLE() 0
static_assert(IS_CUSPARSE12_AVAILABLE() == 1, "Needs CUDA >= 12.0")
#endif

#if IS_CUSPARSE12_AVAILABLE()
...
struct CusparseMatrixMultiplyOp {
  ...
  csrOutput operator()(
      ...
      const cusparseSpGEMMAlg_t alg_type) {
    ...
    int64_t num_prods;
    scalar_t chunk_fraction = 0.2f;
    // cusparseSpGEMMAlg_t alg = CUSPARSE_SPGEMM_DEFAULT;
    // alg =  use_alg3 ? CUSPARSE_SPGEMM_ALG3 : CUSPARSE_SPGEMM_ALG2;
    ...
    // type, the routine returns CUSPARSE_STATUS_ARCH_MISMATCH error
    cudaDeviceProp* prop = at::cuda::getCurrentDeviceProperties();
    TORCH_CHECK(
        prop->major >= 5 &&
            !((10 * prop->major + prop->minor) < 53 &&
              computeType == CUDA_R_16F),
        "sparse_mm: CUDA Float16 requires compute capability >= 53 (current: ",
        prop->major,
        prop->minor,
        ")");
    TORCH_CHECK(
        !(prop->major < 8 && computeType == CUDA_R_16BF),
        "sparse_mm: CUDA BFloat16 requires compute capability >= 80 (current: ",
        prop->major,
        prop->minor,
        ")");
    ...
    if (alg_type == CUSPARSE_SPGEMM_DEFAULT) {
      ...
      TORCH_CUDASPARSE_CHECK(
          cusparseSpGEMM_getNumProducts(spgemmDesc, &num_prods));
...
#else // if not IS_CUSPARSE12_AVAILABLE()

#endif // IS_CUSPARSE12_AVAILABLE()
...
void sparse_sparse_matmul_cuda_kernel(
  ...
  static_assert(
      std::is_same<c10::Half, scalar_t>::value ||
          ...
          std::is_same<float, scalar_t>::value ||
          std::is_same<double, scalar_t>::value,
      "sparse_sparse_matmul_cuda_kernel only supports data type of half, bfloat16, float and double.");
  ...
  csrOutput csr_output =
      op(csr_mat1, csr_mat2, output_values, output_indices, alg_type);
  ...
  alias_into_sparse(result, output_indices, output_values);
}
...
Tensor sparse_sparse_matmul_cuda12(
    ...
    const Tensor& mat2_,
    std::string alg_name) {
  static auto alg_to_type =
      std::unordered_map<c10::string_view, cusparseSpGEMMAlg_t>(
          {{"default", CUSPARSE_SPGEMM_DEFAULT},
           {"alg1", CUSPARSE_SPGEMM_DEFAULT},
           {"alg2", CUSPARSE_SPGEMM_ALG2},
           {"alg3", CUSPARSE_SPGEMM_ALG3}});
  auto alg_type = alg_to_type[alg_name];
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      mat1_.scalar_type(), "sparse_matmul", [&] {
        sparse_sparse_matmul_cuda_kernel<scalar_t>(
            output, mat1_.coalesce(), mat2_.coalesce(), alg_type);
      });
#else
